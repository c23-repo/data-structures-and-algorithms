...
const mapTwoToThe = (arr) => {
  // Solution code here...
  let mapped = arr.map(i => Math.pow(2, i));
  return mapped;
};
...
const charCode = (arr) => {
  // Solution code here...
  let mapped = arr.map(string => string.charCodeAt());
  return mapped;
};
...
const extractAbilities = (arr) => {
  // Solution code here...
  let abilityName = arr.map(i => i.ability.name);
  return abilityName;
};
...
const extractStats = (arr) => {
  // Solution code here...
  let statArr = arr.map( obj => {
    let newObj = {};
    newObj['name'] = obj.stat.name;
    newObj['total'] = (obj.effort + obj.baseStat);
    return newObj;
  });
  return statArr;
