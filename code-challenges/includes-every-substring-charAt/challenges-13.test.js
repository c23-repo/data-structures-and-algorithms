...
const standardizePhoneNumbers = (arr) => arr.map(element => element.substring(1,4)+element.substring(6,9)+element.substring(10,14));
...
const onlyOddChars = (str) => str.split('').filter((element,index) => index%2).join('');
...
const findAnything = (arr, target) => arr.filter(element => element.includes(target));
...
const unenrollBrook = (arr) => arr.map(twoD => twoD.filter(element => !element.includes('Brook')));
